...
let cidades = ["Curitiba", "Manaus", "Belo Horizonte", "Salvador", "Fortaleza"]
...
console.log(`total de cidades no vetor: ${cidades.length}`);
...
for (let i = 0; i < listaNumeros.length; i++) {
    ...
    if (numeroAtual % 2 === 0) {
        paresVetor.push(numeroAtual);
    } else {
         imparesVetor.push(numeroAtual);
    }
}
...
console.log(`Invertido 2 (com função): ${vetorInvertido2}`);
console.log("_______________________________");
...
// → Seu código aqui:
let produtos = []
let user = Keyboard.questionInt("quantos produtos deseja cadastrar ")

for (let i = 0; i < user; i++){
    let nome = Keyboard.question(`nome do produto ${i + 1}: `)
    produtos.push(nome);
}
...
// → Seu código aqui:
const alunos = ["Ana", "Bruno", "Carla", "Diego", "Eva"];
let professor = Keyboard.question("digite o nome de um aluno ");
let achou = -1;

for (let i = 0; i < alunos.length; i++) {
    if (alunos[i] === professor) {
        achou = i;
        break; 
    }
}
...
if (achou === -1) {
    console.log(`${professor} nao foi encontrado(a).`);
} else {
    console.log(`${professor} está matriculado(a) (índice ${achou}).`);
}
console.log("_______________________________");
